...
export function wiresFrom(hub: Box | null, tiles: Map<string, Box>): Wire[] {
  ...
  for (const [target, tile] of tiles) {
    ...
    const lane = round(
      tile.x < hub.x + hub.width
        ? Math.max(hub.y + hub.height + STUB, tile.y - LANE)
        : tile.y - LANE,
    );
    wires.push({
      target,
...
/**
 * The line an arriving order travels: from the top of the box, where the button is,
 * straight down into the mediator's top edge.
 *
 ...
 */
export function entryWire(hub: Box | null): Wire | null {
  if (hub === null) return null;
  const x = round(hub.x + hub.width / 2);
